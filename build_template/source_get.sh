...
# INSTALL EXTEND LIB
apt-get -y install libsasl2-dev libldap-dev
...
# Wait for all background wget processes to finish
wait

# Set environment variables for the build process
echo "/usr/local/subversion/lib" >> /etc/ld.so.conf.d/subversion.conf
LD_LIBRARY_PATH=/usr/local/subversion/lib
LD_RUN_PATH=/usr/local/subversion/lib

if [ "${BUILD_TARGET}" = "httpd_svn" ]; then
    echo "/usr/local/httpd/lib" >> /etc/ld.so.conf.d/subversion.conf
    LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/usr/local/httpd/lib
    LD_RUN_PATH=${LD_RUN_PATH}:/usr/local/httpd/lib
fi
